-- =============================================================================
-- Thakkar Medico — V88: Product schemes + optional free qty on orders (POS / invoice)
--
-- 1. products.scheme_buy_qty / scheme_free_qty — hint only (e.g. buy 10 get 1 free)
-- 2. order_items.free_qty — free units on a line (not charged; still deducts stock)
-- 3. place_order accepts free_qty or free_quantity in p_items JSON
-- =============================================================================

BEGIN;

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS scheme_buy_qty integer,
  ADD COLUMN IF NOT EXISTS scheme_free_qty integer;

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS free_qty integer NOT NULL DEFAULT 0;

ALTER TABLE public.products
  DROP CONSTRAINT IF EXISTS products_scheme_qty_check;

ALTER TABLE public.products
  ADD CONSTRAINT products_scheme_qty_check CHECK (
    (scheme_buy_qty IS NULL AND scheme_free_qty IS NULL)
    OR (
      scheme_buy_qty IS NOT NULL AND scheme_free_qty IS NOT NULL
      AND scheme_buy_qty > 0 AND scheme_free_qty > 0
    )
  );
...
BEGIN
  ...
  LOOP
    ...
    v_free_qty := COALESCE(
      NULLIF(v_item ->> 'free_qty', '')::integer,
      NULLIF(v_item ->> 'free_quantity', '')::integer,
      0
    );
    IF v_free_qty < 0 THEN
      RAISE EXCEPTION 'invalid_quantity'
        USING HINT = 'free_qty cannot be negative';
    END IF;
    ...
    IF v_stock < (v_base_qty + v_free_qty) THEN
      RAISE EXCEPTION 'insufficient_stock'
        USING HINT = format(
          'Product %s: requested %s paid + %s free but only %s in stock',
          v_product_id, v_base_qty, v_free_qty, v_stock
        );
    END IF;
  ...
  LOOP
    ...
    v_remaining    := v_base_qty + v_free_qty;
    v_allocations  := '[]'::jsonb;
    ...
    INSERT INTO order_items (order_id, product_id, qty, free_qty, unit_price, gst_percent, line_total, batch_allocations)
    VALUES (v_order_id, v_product_id, v_base_qty, v_free_qty, v_unit_price, v_gst_pct, v_line_total, v_allocations);

    INSERT INTO stock_history (product_id, change, reason)
    VALUES (v_product_id, -(v_base_qty + v_free_qty), 'Order ' || v_order_number);
  END LOOP;
