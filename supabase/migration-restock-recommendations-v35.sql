-- =============================================================================
-- Thakkar Medico — Restock recommendations RPC (v35)
-- Tier 1 "Time to restock" rail: interval-based reorder urgency from order history
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    (EXTRACT(EPOCH FROM (now() - i.last_ordered_at)) / 86400)
      / GREATEST(i.avg_interval_days, 1) AS restock_urgency
  FROM intervals i
...
-- Test checklist
-- =============================================================================
-- □ Retailer with 2+ orders same product ~2 weeks apart — appears at 80%+ interval
-- □ Badge urgency: overdue (>=1.5), due now (1.0–1.5), due soon (0.8–1.0)
-- □ OOS product excluded
-- □ New / single-order retailer — empty set
-- □ Admin rpc for another user — succeeds; non-admin other uuid — access_denied
-- □ SQL editor without JWT — not_authenticated
