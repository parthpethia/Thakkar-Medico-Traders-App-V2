...
-- 4. ITEM 4 (P1): Restores 50-point location history breadcrumb query in
--    get_public_order_tracking and get_order_tracking_bundle.
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- SECTION 2: Consolidated get_delivery_health_summary(p_canary_only boolean)
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_caller_role NOT IN ('admin', 'delivery') THEN
  ...
  -- 1. Fetch canary rider list for cohort filtering
  SELECT array_agg(rider_id)
    INTO v_canary_rider_ids
    FROM public.canary_rider_flags
   WHERE feature_set = 'delivery_flow_v2'
     AND enabled = true;

  -- Active canary riders count
  SELECT COUNT(*)::integer
    INTO v_canary_riders_count
    FROM public.canary_rider_flags
   WHERE feature_set = 'delivery_flow_v2'
     AND enabled = true;
...
-- -----------------------------------------------------------------------------
-- SECTION 3: Aligned & Hardened get_public_order_tracking
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  BEGIN
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    -- Non-fatal fallback for rate limits
    NULL;
  END;

  -- Lookup Order strictly by UUID or Order Number
  IF p_order_identifier ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
    v_order_uuid := p_order_identifier::uuid;
    SELECT * INTO v_order FROM public.orders WHERE id = v_order_uuid LIMIT 1;
  ELSE
    SELECT * INTO v_order
      FROM public.orders
     WHERE order_number ILIKE trim(p_order_identifier)
        OR order_number ILIKE 'ORD-' || trim(p_order_identifier)
     ORDER BY created_at DESC
     LIMIT 1;
  END IF;
  ...
  IF v_order.id IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Order not found. Please verify the order number or link.'
    );
  END IF;
  ...
  -- Load Location History Breadcrumbs (Restored 50 points)
